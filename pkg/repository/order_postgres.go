...
func (r *OrderPostgres) GetAllForPhotographer(photographerId string) ([]models.Order, error) {
	var orders []models.Order
	err := db.Table(ordersTable).Where("photographer_id = ? AND owner = ?", photographerId, "photographer").Find(&orders).Error
...
func (r *OrderPostgres) GetAllForDesigner(designerId string) ([]models.Order, error) {
	var orders []models.Order
	err := db.Table(ordersTable).Where("designer_id = ? AND owner = ?", designerId, "designer").Find(&orders).Error
